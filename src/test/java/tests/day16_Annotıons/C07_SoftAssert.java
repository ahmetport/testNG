...
public class C07_SoftAssert extends TestBase {
    ...
    public void test01() {
        ...
        //LİST STRİNGE CEVİRELİM
        List<String>actualOpsiyonlarınListesi=new ArrayList<String>();
        for (WebElement each:optionList
             ) {
            actualOpsiyonlarınListesi.add(each.getText());
        }
        //System.out.println(actualOpsiyonlarınListesi);
       List<String>strOpsiyonlarListesi=new ArrayList<String>(Arrays.asList("Select One", "Australia (dollar)",
                "Canada (dollar)","Switzerland (franc)","China (yuan)","Denmark (krone)","Eurozone (euro)","Great Britain (pound)",
                "Hong Kong (dollar)","Japan (yen)","Mexico (peso)","Norway (krone)","New Zealand (dollar)","Sweden (krona)",
                "Singapore (dollar)","Thailand (baht)"));
       Collections.sort(actualOpsiyonlarınListesi);
       Collections.sort(strOpsiyonlarListesi);
       softAssert.assertEquals(actualOpsiyonlarınListesi,strOpsiyonlarListesi,"eşit degil");
